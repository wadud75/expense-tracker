...
function buildProductName(fields) {
  return [fields.productName, fields.brandName, fields.variantName]
    .map(normalizeText)
    .filter(Boolean)
    .join(" - ");
}

function buildInvoiceNo(date) {
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  const sequence = String(Math.floor(Math.random() * 100000)).padStart(5, "0");
  return `SL-${year}${month}${day}-${sequence}`;
}
...
export async function createSalesBatch({
  ...
  const invoiceNo = buildInvoiceNo(now);
  const invoiceTotal = normalizedItems.reduce(
    (total, item) => total + item.quantity * item.unitPrice,
    0,
  );
  const paidValue = normalizeNumber(paidAmount, invoiceTotal);
...
export async function getStockSnapshot() {
  const { products, stockMovements } = await getCollections();
  const productRows = await products.find({}).sort({ updatedAt: -1 }).toArray();
