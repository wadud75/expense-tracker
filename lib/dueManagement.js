...
function createDueRecord({
  ...
  return {
    ...
    updatedAt: parsedUpdatedAt || parsedCreatedAt,
...
function buildSalesDueRecords(rows, customerMap = {}) {
  const invoices = rows.reduce((summary, sale) => {
    const invoiceNo = normalizeText(sale.invoiceNo) || sale._id.toString();
    const customer = customerMap[normalizeText(sale.customerName).toLowerCase()] || {};
    ...
    if (!summary[invoiceNo]) {
      summary[invoiceNo] = {
        ...
        products: [],
        customerAddress: normalizeText(sale.customerAddress) || normalizeText(customer.address),
        customerPhone: normalizeText(sale.customerPhone) || normalizeText(customer.phone),
...
function buildPurchaseDueRecords(rows) {
  return rows.map((purchase) => {
    ...
    return createDueRecord({
      ...
      updatedAt: purchase.updatedAt || purchase.createdAt,
      note: purchase.notes,
      paymentHistory: mapPaymentHistory(
...
export async function getDueManagementSnapshot() {
  const { sales, purchases, dueEntries, customers } = await getCollections();

  const [salesRows, purchaseRows, manualRows, customerRows] = await Promise.all([
  ...
  const customerMap = customerRows.reduce((summary, customer) => {
    const key = normalizeText(customer.name).toLowerCase();
    if (key) {
      summary[key] = customer;
    }
    return summary;
  }, {});

  const records = [
    ...buildSalesDueRecords(salesRows, customerMap),
    ...buildPurchaseDueRecords(purchaseRows),
    ...buildManualDueRecords(manualRows),
  ].sort((left, right) => new Date(left.createdAt || left.dueDate).getTime() - new Date(right.createdAt || right.dueDate).getTime());
